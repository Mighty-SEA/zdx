...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Tambah Komoditas Baru</h1>
                <p class="mt-2 text-sm text-gray-600">
                    Tambahkan jenis komoditas baru untuk ditampilkan di halaman komoditas website
                </p>
            </div>
            <div>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <form action="{{ route('admin.commodity.store') }}" method="POST" enctype="multipart/form-data" id="addCommodityForm">
            @csrf
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#FF6000] focus:ring focus:ring-[#FF6000] focus:ring-opacity-50">
                        @error('name')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-[#FF6000] focus:ring focus:ring-[#FF6000] focus:ring-opacity-50">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
